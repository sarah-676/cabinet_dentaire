/**
 * pages/dentiste/MonComptePage.jsx
 * ===================================
 * Même structure que MonComptePage admin.
 * Champs modifiables supplémentaires : specialite, numero_ordre.
 */
...
const S = {
  ...
  tab:       (a) => ({ padding: "0.6rem 1.25rem", fontWeight: a ? "600" : "400", color: a ? "#2563eb" : "#64748b", background: "none", border: "none", borderBottom: a ? "2.5px solid #2563eb" : "2.5px solid transparent", cursor: "pointer", fontSize: "0.9rem", marginBottom: "-1.5px" }),
  card:      { backgroundColor: "#fff", borderRadius: "12px", padding: "1.75rem", maxWidth: "560px", boxShadow: "0 1px 3px rgba(0,0,0,0.07)" },
  ...
  btn:       { padding: "0.65rem 1.5rem", backgroundColor: "#2563eb", color: "#fff", border: "none", borderRadius: "8px", fontSize: "0.9rem", fontWeight: "600", cursor: "pointer" },
  ...
  avatar:    { width: "60px", height: "60px", borderRadius: "50%", backgroundColor: "#dbeafe", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.375rem", fontWeight: "700", color: "#1d4ed8" },
};
...
function TabProfil() {
  ...
  const [form, setForm] = useState({
    ...
    phone: user?.phone ?? "", specialite: user?.specialite ?? "",
  ...
  const submit = async (e) => {
    ...
    try { const { data } = await updateProfile(form); updateUser(data); setOk(true); }
      ...
      if (d.detail) setErrors({ _g: d.detail });
      else { const m = {}; Object.entries(d).forEach(([k,v]) => { m[k] = Array.isArray(v) ? v[0] : v; }); setErrors(m); }
    } finally { setSaving(false); }
  };

  const initials = `${user?.first_name?.[0]??''}${user?.last_name?.[0]??''}`.toUpperCase()||'D';
  return (
    <div style={S.card}>
      <div style={S.avatarRow}>
        ...
        <div>
          ...
          {user?.specialite && <p style={{ margin:0, fontSize:"0.78rem", color:"#94a3b8" }}>{user.specialite}</p>}
        </div>
      </div>
      ...
      <form onSubmit={submit}>
        <div style={S.row}>
          <div style={S.formGroup}>
            ...
            <input type="text" value={form.first_name} onChange={e => set("first_name",e.target.value)} style={{...S.input,...(errors.first_name?S.inputErr:{})}} />
            {errors.first_name && <p style={S.fieldErr}>{errors.first_name}</p>}
          </div>
          <div style={S.formGroup}>
            <label style={S.label}>Nom</label>
            <input type="text" value={form.last_name} onChange={e => set("last_name",e.target.value)} style={{...S.input,...(errors.last_name?S.inputErr:{})}} />
            {errors.last_name && <p style={S.fieldErr}>{errors.last_name}</p>}
          </div>
        </div>
        <div style={S.formGroup}>
          <label style={S.label}>Téléphone</label>
          <input type="tel" value={form.phone} onChange={e => set("phone",e.target.value)} style={S.input} />
        </div>
        <div style={S.row}>
          <div style={S.formGroup}>
            <label style={S.label}>Spécialité</label>
            <input type="text" value={form.specialite} onChange={e => set("specialite",e.target.value)} style={S.input} placeholder="Orthodontie" />
          </div>
          <div style={S.formGroup}>
            <label style={S.label}>N° d'ordre</label>
            <input type="text" value={form.numero_ordre} onChange={e => set("numero_ordre",e.target.value)} style={S.input} />
          </div>
        </div>
        <button type="submit" disabled={saving} style={{...S.btn, opacity:saving?0.65:1}}>
...
function TabSecurite() {
  ...
  return(
    <div style={S.card}>
      ...
      {errors._g&&<div style={S.errorBox}>⚠ {errors._g}</div>}
      <form onSubmit={submit}>
        <div style={S.formGroup}><label style={S.label}>Mot de passe actuel</label><input {...fp("current_password")} />{errors.current_password&&<p style={S.fieldErr}>{errors.current_password}</p>}</div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"0.5rem",marginBottom:"1.25rem"}}>
          <input type="checkbox" id="sp" checked={show} onChange={e=>setShow(e.target.checked)} />
          <label htmlFor="sp" style={{fontSize:"0.85rem",color:"#64748b",cursor:"pointer"}}>Afficher les mots de passe</label>
        </div>
